...
namespace AssetMatchTool
{
    public static class Check
    {
        ...
        public static bool CheckModel(byte[] datafile, uint address, int numhierarchy, ModelFormat modelfmt, uint ImageBase, bool landtable = false, bool verbose = false, NJS_OBJECT obj = null)
        {
            ...
            switch (modelfmt)
            {
                ...
                case ModelFormat.BasicDX:
                    ...
                    if (attach != 0 && attach < ImageBase) return false;
                    ...
                    if (obj != null)
                    {
                        ...
                        if (obj.Scale.X != scl.X) return false;
                        if (obj.Scale.Y != scl.Y) return false;
                        if (obj.Scale.Z != scl.Z) return false;
                        if (obj.Position.X != pos.X) return false;
                        if (obj.Position.Y != pos.Y) return false;
                        if (obj.Position.Z != pos.Z) return false;
                    }
                    ...
                    if (numhierarchy != -1 && child != 0)
                    {
                        ...
                        {
                            numhierarchy++;
                            return CheckModel(datafile, child - ImageBase, numhierarchy, modelfmt, ImageBase);
                        }
                        else
                            return CheckModel(datafile, child - ImageBase, -1, modelfmt, ImageBase);
                    }
                    if (numhierarchy != -1 && sibling != 0)
                    {
                        ...
                        {
                            numhierarchy++;
                            return CheckModel(datafile, sibling - ImageBase, numhierarchy, modelfmt, ImageBase);
                        }
                        else
                            return CheckModel(datafile, sibling - ImageBase, -1, modelfmt, ImageBase);
                    }
                ...
                case ModelFormat.Chunk:
                    ...
                    if (child != 0 && child < ImageBase) return false;
                    if (sibling != 0 && sibling < ImageBase) return false;
                    if (numhierarchy != 0 && child != 0 && !CheckModel(datafile, child - ImageBase, -1, modelfmt, ImageBase)) return false;
                    if (numhierarchy != 0 && sibling != 0 && !CheckModel(datafile, sibling - ImageBase, -1, modelfmt, ImageBase)) return false;
                ...
                case ModelFormat.GC:
                    ...
                    if (child != 0 && child < ImageBase) return false;
                    if (sibling != 0 && sibling < ImageBase) return false;
                    if (numhierarchy != -1 && child != 0 && !CheckModel(datafile, child - ImageBase, -1, modelfmt, ImageBase)) return false;
                    if (numhierarchy != -1 && sibling != 0 && !CheckModel(datafile, sibling - ImageBase, -1, modelfmt, ImageBase)) return false;
                    if (attach == 0 && flags == 0) return false;
                    if (attach == 0 && child == 0 && sibling == 0) return false;
        ...
        public static bool CheckLandTable(byte[] datafile, uint address, LandTableFormat landfmt, uint ImageBase)
        {
            ...
            switch (landfmt)
            {
                ...
                case LandTableFormat.SADX:
                    ...
                    if (!CheckModel(datafile, ObjAddr - ImageBase, -1, modelfmt, ImageBase, true, false)) return false;
                    break;
                case LandTableFormat.SA2:
                case LandTableFormat.SA2B:
                    COLCount = ByteConverter.ToInt16(datafile, (int)address);
